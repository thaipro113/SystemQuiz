...
namespace SystemQuiz.Services
{
    public class QuestionService : IQuestionService
    {
        ...
        public async Task<bool> DeleteQuestionAsync(int id)
        {
            ...
            // Load ResultDetails that reference this question to allow ClientSetNull to work
            var resultDetails = await _context.ResultDetails
                .Where(rd => rd.QuestionId == id)
                .ToListAsync();

            // Set QuestionId to null for all ResultDetails that reference this question
            foreach (var rd in resultDetails)
            {
                rd.QuestionId = null;
                rd.Question = null;
            }

            // Also handle ResultDetails that reference answers of this question
            var answerIds = question.Answers.Select(a => a.Id).ToList();
            var resultDetailsWithAnswers = await _context.ResultDetails
                .Where(rd => answerIds.Contains(rd.AnwserId ?? 0) || answerIds.Contains(rd.SelectedAnwserId ?? 0))
                .ToListAsync();

            foreach (var rd in resultDetailsWithAnswers)
            {
                if (answerIds.Contains(rd.AnwserId ?? 0))
                {
                    rd.AnwserId = null;
                    rd.Anwser = null;
                }
                if (answerIds.Contains(rd.SelectedAnwserId ?? 0))
                {
                    rd.SelectedAnwserId = null;
                }
            }
